...
def csv_locs(tic):
    ...
    given ticker `tic`. 返回一个字典，其中包含一个给定的ticker `tic`的源CSV文件的位置。
    ...
    -------
    dict
        A dictionary with the following keys: 一个有以下键的字典：
        'rec_csv': The complete path to the CSV with the recommendation data 包含推荐数据的CSV的完整路径
        'prc_csv': The complete path to the CSV with the price data 包含价格数据的CSV的完整路径
    """
